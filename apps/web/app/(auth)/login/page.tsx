...
import { z } from 'zod'
import { Eye, EyeOff, Loader2, Shield, Sparkles, RefreshCw } from 'lucide-react'
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (res.ChallengeName === 'SOFTWARE_TOKEN_MFA') {
        setSession(res.Session || ''); setMfa(true)
      } else {
        const role = res.role
        ...
        <div className="glass p-8">
          ...
          {!mfaRequired ? (
            ...
            <div className="space-y-4">
              ...
              <button onClick={() => setMfa(false)} className="w-full text-sm text-slate-400 hover:text-white transition-colors">← Back to login</button>
            </div>
          )}
        </div>

        <p className="text-center text-xs text-slate-500 mt-6">Protected by AWS Cognito · TLS 1.3 · MFA Available</p>
      </div>
    </div>
  )
}
