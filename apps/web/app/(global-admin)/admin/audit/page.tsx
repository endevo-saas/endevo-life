...
export default function AdminAuditPage() {
  ...
  const actionColor = (action: string) => {
    ...
    if (action?.includes('UPDATE') || action?.includes('UPDATED')) return 'bg-yellow-500/10 text-yellow-400'
    if (action?.includes('CREATE') || action?.includes('CREATED')) return 'bg-green-500/10 text-green-400'
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">{filtered.length} entries (last 100)</p>
          </div>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      No audit logs found. Actions will appear here as users perform operations.
                    </td>
                  </tr>
                ) : filtered.map(log => (
                  <tr key={log.auditId} className="hover:bg-white/3 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-500 font-mono">{log.tenantId}</td>
                  </tr>
                ))}
              </tbody>
            </table>
