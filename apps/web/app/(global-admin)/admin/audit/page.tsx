...
import {
  ...
  Shield, MapPin, Monitor, Wifi, Filter, Download,
...
const ACTION_COLOR: Record<string, string> = {
  ...
  MFA_SUCCESS:            'bg-emerald-500/10 text-emerald-400 border-emerald-500/20',
  MFA_FAILED:             'bg-orange-500/10 text-orange-400 border-orange-500/20',
  ...
  TENANT_CREATED:         'bg-brand-500/10 text-brand-400 border-brand-500/20',
  TENANT_UPDATED:         'bg-yellow-500/10 text-yellow-400 border-yellow-500/20',
  TENANT_DELETED:         'bg-red-500/10 text-red-400 border-red-500/20',
...
function parseCity(device?: string): string {
  return ''
}
...
export default function AdminAuditPage() {
  ...
  const categories = ['ALL', 'LOGIN', 'PASSWORD', 'TENANT', 'USER', 'EMPLOYEE', 'REGISTER', 'INVITE']
  ...
  const exportCSV = () => {
    ...
    a.download = `endevo-audit-${new Date().toISOString().slice(0,10)}.csv`
  ...
  const loginBlocks    = logs.filter(l => l.action === 'LOGIN_BLOCKED').length

  return (
    <div className="min-h-screen p-6">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-0 w-80 h-80 bg-brand-600/6 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">{filtered.length} of {logs.length} events · IP + Device tracking enabled</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            { label: 'IP Blocks',         value: loginBlocks,      color: 'text-orange-400', bg: 'from-orange-600/20 to-orange-800/10 border-orange-500/30' },
          ].map(s => (
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search actor, action, details, IP address..."
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-brand-500/50" />
            </div>
              ...
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">
                    <Wifi className="w-3.5 h-3.5 inline mr-1 text-brand-400" />IP Address
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">
                          ...
                          <td colSpan={7} className="px-4 py-3">
                            <div className="grid grid-cols-3 gap-4 text-xs">
                              ...
                              <div>
                                ...
                                <p className="font-mono text-brand-300">{log.ip_address || '—'}</p>
                                <p className="text-slate-500 mt-1">Tenant ID</p>
                                <p className="font-mono text-slate-300">{log.tenantId || '—'}</p>
                              </div>
                              <div>
