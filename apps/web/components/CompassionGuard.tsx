 ...
 * 3. A compassion_mode cookie is set
 *
...
export default function CompassionGuard() {
  useEffect(() => {
    ...
    const compassionCookie = Cookies.get('compassion_mode')
    ...
    const isForcedCompassion = compassionCookie === 'true'
    ...
    if (isDelegate || isBereavementPath || isForcedCompassion) {
      ...
      if (currentTheme && currentTheme !== 'compassion') {
        localStorage.setItem('endevo-theme-before-compassion', currentTheme)
      }
      document.documentElement.setAttribute('data-theme', 'compassion')
      document.documentElement.classList.add('light-mode')
      localStorage.setItem('endevo-theme', 'compassion')
    }
  }, [])
